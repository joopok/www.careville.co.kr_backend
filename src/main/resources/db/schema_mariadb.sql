...
CREATE TABLE `TB_PRODUCT` (
    ...
    `FILE_SEQ1`        INT(11) DEFAULT NULL COMMENT '대표이미지1 파일번호 (TB_FILE 참조)',
    `FILE_SEQ2`        INT(11) DEFAULT NULL COMMENT '대표이미지2 파일번호 (TB_FILE 참조)',
    `FILE_SEQ3`        INT(11) DEFAULT NULL COMMENT '대표이미지3 파일번호 (TB_FILE 참조)',
    `FILE_SEQ4`        INT(11) DEFAULT NULL COMMENT '대표이미지4 파일번호 (TB_FILE 참조)',
    `FILE_SEQ5`        INT(11) DEFAULT NULL COMMENT '대표이미지5 파일번호 (TB_FILE 참조)',
    `FILE_SEQ6`        INT(11) DEFAULT NULL COMMENT '대표이미지6 파일번호 (TB_FILE 참조)',
    ...
    `FEATURES`         LONGTEXT DEFAULT NULL COMMENT '카드표시정보/특징 (JSON)',
    `SERVICE_EFFECTS`  LONGTEXT DEFAULT NULL COMMENT '서비스효과 (JSON)',
    `WORK_PROCESS`     LONGTEXT DEFAULT NULL COMMENT '작업과정 (JSON)',
    ...
    `POPULAR_YN`       CHAR(1) DEFAULT 'N' COMMENT '인기상품여부 (Y/N)',
    `DEL_YN`           CHAR(1) DEFAULT 'N' COMMENT '삭제여부 (Y/N, 논리삭제)',
    `REG_USER_ID`      VARCHAR(50) DEFAULT NULL COMMENT '등록자 ID',
    `REG_DT`           DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '등록일시',
    `MOD_USER_ID`      VARCHAR(50) DEFAULT NULL COMMENT '수정자 ID',
    ...
    KEY `IDX_PRODUCT_POPULAR` (`POPULAR_YN`, `DEL_YN`),
    KEY `IDX_PRODUCT_FILE1` (`FILE_SEQ1`),
    KEY `IDX_PRODUCT_FILE2` (`FILE_SEQ2`),
    KEY `IDX_PRODUCT_FILE3` (`FILE_SEQ3`),
    KEY `IDX_PRODUCT_FILE4` (`FILE_SEQ4`),
    KEY `IDX_PRODUCT_FILE5` (`FILE_SEQ5`),
    KEY `IDX_PRODUCT_FILE6` (`FILE_SEQ6`),
...
CREATE TABLE `TB_CONSULTATION` (
    ...
    `COMP_NM`      VARCHAR(100) DEFAULT NULL COMMENT '업체명/회사명',
    ...
    `HOPE_TIME`    VARCHAR(20) DEFAULT NULL COMMENT '희망시간 (예: 09:00 - 10:00)',
...
CREATE TABLE `TB_CASE` (
    ...
    `FILE_SEQ`   INT(11) DEFAULT NULL COMMENT '대표이미지1 파일번호 (TB_FILE 참조)',
    `FILE_SEQ2`  INT(11) DEFAULT NULL COMMENT '대표이미지2 파일번호 (TB_FILE 참조)',
    `HASHTAG`    VARCHAR(500) DEFAULT NULL COMMENT '해시태그 (쉼표 구분)',
    `SERVICE_CD` VARCHAR(3) DEFAULT NULL COMMENT '서비스코드 (TB_CATEGORY 참조)',
    `AREA_TYPE`  VARCHAR(50) DEFAULT NULL COMMENT '면적 (예: 32평, 25평형)',
