    ...
    function html5Upload() {	
		...
		var sUploadURL	= '/editorImgUploader.do'; 	//upload URL
		var formData	= new FormData();
		
		for(var j=0, k=0; j < nImageInfoCnt; j++) {
			formData.append("files", htImageInfo['img'+j]);
		}
		
		fetch(sUploadURL,{method:"POST",body:formData}
		).then(res => res.json()
		).then(data => {
    ...
    function html5Upload() {	
		...
		sUploadURL= '/editorImgUploader.do'; 	//upload URL
    ...
    */
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	...
    	var oAjax = jindo.$Ajax('/editorImgUploader1.do', {
			type: 'xhr',
			method : "post",
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/common/editorImgUploadEi',	//샘플 URL입니다.
